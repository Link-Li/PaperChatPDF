...
data_json = {"pdf_id": "1"}
response = requests.get(url, data=json.dumps(data_json), headers=headers)
...
# 测试提取pdf的abstract的关键词
url = "http://127.0.0.1:8001/pdf/extract_abstract_core_keywords"
data_json = {"pdf_id": "1"}
response = requests.get(url, data=json.dumps(data_json), headers=headers)
print(response)
print(json.loads(response.text))

llm_api = "http://192.168.31.137:9001/pdf/llm_chat"
message_list = [
...
generate_info = requests.post(url=llm_api, json=data_json, headers=headers)
...
# 测试提取pdf的title的关键词
url = "http://127.0.0.1:8001/pdf/extract_title_core_keywords"
data_json = {"pdf_id": "1"}
response = requests.get(url, data=json.dumps(data_json), headers=headers)
print(response)
print(json.loads(response.text))